...
export default function Wisher() {
    ...
    const independenceDayMessages = [
      ...
      const initialMessage = `Celebrating 77 years of freedom! 🇮🇳 Let's embrace our unity and diversity. Together, we've sculpted a nation where dreams take flight and aspirations know no bounds. #IndependenceDay2023 #ProudToBeIndian #77YearsOfFreedom`;
      ...
      const handleCopyLink = () => {
        if (wishId) {
          ...
          linkInput.value = `https://independence-day-wisher.vercel.app/wish/${wishId}`;
      ...
      const handleShare = () => {
        const data = {
          title: "Celebrate India's 77th Independence Day",
          text: "Join us in celebrating India's 77th Independence Day with heartfelt wishes and messages. Send wishes to your loved ones and share the patriotic spirit!",
          url: `https://independence-day-wisher.vercel.app/wish/${wishId}`,
        };
      
        if (navigator.share) {
          Swal.fire({
            title: 'Share Independence Day Wishes',
            text: 'Are you sure you want to share this message?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
              navigator.share(data)
                .then(() => {
                  Swal.fire('Shared!', 'Your message has been shared successfully.', 'success');
                })
                .catch(error => {
                  Swal.fire('Error', 'Oops! Something went wrong while sharing.', 'error');
                  console.error('Error sharing:', error);
                });
            }
          });
        } else {
          Swal.fire('Share API not supported', 'error');
        }
      }
      ...
      <div className={styles.wisherLeft}>
        ...
        {formSubmitted && (
          <div className={styles.CopyLink}>
            <input
              type="url"
              value={`https://independence-day-wisher.vercel.app/wish/${wishId}`}
              readOnly
            />
